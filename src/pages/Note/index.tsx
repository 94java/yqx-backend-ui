...
import { Button, Dropdown, Popconfirm, Table, Tag, message, Modal, Radio, RadioChangeEvent } from "antd";
...
const columns: ProColumns<NOTE.NoteItem>[] = [
  ...
  },
  {
    ...
    dataIndex: "title",
    render: (_, record) => (
      <Button type="link" href={"/note/content?id=" + record.id}>
        {record.title}
      </Button>
    ),
  },
  ...
  },
  {
    ...
    render: (text, record, _, action) => [
      <a key="settings" href={"/note/content?id=" + record.id}>编辑</a>,
      <Popconfirm
        title="你确认删除此条记录吗？"
        onConfirm={async () => {
  ...
  return (
    <ProTable<CATEGORY.CategoryItem>
      ...
      toolBarRender={() => [
        ...
        >
          <Radio.Group style={{marginTop:'10px',margin:'auto'}}
            options={[
              { label: "待审核", value: "0" },
              { label: "草稿", value: "2" },
              { label: "已发布", value: "1"},
            ]}
            onChange={({ target: { value } }: RadioChangeEvent) => {setStatusChange(value)}}
            value={statusChange}
            optionType="button"
            buttonStyle="solid"
          />
